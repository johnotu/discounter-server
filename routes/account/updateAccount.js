...
const updateAccount = (req, res, next) => {
  ...
  if (req.body.password || req.body.token || req.body.tokenExpiry || req.body.salt) return res.status(403).json({ msg: `Invalid update parameter(s) ${['password', 'token', 'tokenExpiry', 'salt'].filter(param => req.body[param]).join(', ')}` });
